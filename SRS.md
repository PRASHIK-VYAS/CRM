...
**Project Name:** EduBridge Enterprise  
...
**Module: Company Intelligence** — A centralized record of all corporate entities to prevent data loss.

- **Sub-module: Company Management**
  - **FR-001** The system SHALL allow adding/editing companies with industry and hiring history.
  - **FR-002** The system SHALL allow importing contacts via CSV/Excel.
- **Sub-module: Contact Matrix**
  - **FR-003** The system SHALL associate multiple recruiters with a single company.

#### 3.1.2 Phase 2: Transactional

**Module: Placement Drive Manager** — Management of the job cycle from posting to offer.

- **Sub-module: Job Lifecycle**
  - **FR-004** The system SHALL allow TPOs to track students through Applied → Shortlisted → Offered.
- **Sub-module: Offer Ledger**
  - **FR-005** The system SHALL record CTC and joining dates for every offer.

#### 3.1.3 Phase 3: Strategic

**Module: Corporate Outreach** — Automating the bridge between college and industry.

- **Sub-module: Email Broadcaster**
  - **FR-006** The system SHALL send bulk emails using predefined templates with variables.
...
- **Sub-module: Health Score**
  - **FR-010** The system SHALL calculate a company health score based on visits and placements.
...
- **Sub-module: Profile Mgmt**
  - **FR-011** The system SHALL allow students to maintain a digital resume.
- **Sub-module: Job Application**
  - **FR-012** The system SHALL allow students to apply to jobs they are eligible for.
...
- **Company → Contact:** (1:N) — One company has multiple recruiter contacts.
- **Student → Application → Job:** (N:N) — Multiple students can apply to multiple jobs.
- **Alumni → Company:** (N:1) — An alumnus is mapped to one current company.
